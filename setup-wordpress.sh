...
check_command \
    "Failed to install required packages (mariadb-server, mariadb-client, php8.2-mysql)" \
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y mariadb-server mariadb-client php8.2-mysql
